...
public class WifiObservation extends Observation {
	...
	public WifiObservation(double timestamp, int size) {
		this.timestamp = timestamp;
		measurements = new HashMap<Integer,Integer>((int)((float)size/0.75f));
		num_observations = 0;
	}
	
	public void addObservation(int ap_id, int signal_strength) {
	...
	public double distanceFrom(Observation other) {
		WifiObservation observation = (WifiObservation)other;
		...
		for (Map.Entry<Integer,Integer> entry : p1.measurements.entrySet()) {
		...
		if ((float)num_common / (float)p2.num_observations > ETA) {
			dist = Math.sqrt((1.0f/(float)num_common)*dist);
		}
		else {
	...
	public String toString() {
		...
		for (Entry<Integer, Integer> entry : measurements.entrySet()) {
			sb.append("\tWapId: " + entry.getKey() + ", Strength: " + entry.getValue() + "\n");
		}
		return sb.toString();
